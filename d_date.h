...
//** My code FROM  / Ali **********************************************************************
bool date::operator>(const date&rghty)
{
	if (getYear() > rghty.year)
		return true;
	else if (getYear() < rghty.year)
		return false;
	else if (getMonth() > rghty.month)
		return true;
	else if (getMonth() < rghty.month)
		return false;
	else if (getDay() > rghty.day)
		return true;
	else
		return false;
}
...
ostream& operator<<(ostream& ostr, const date& rghty)
{
	ostr << rghty.year << "/" << rghty.month << "/" << rghty.day << endl;
	return ostr;
}

istream& operator>>(istream& istr, date& rghty)
{
	int day , mon, year;
	char slash;
	istr >> mon >> slash >> day >> slash >> year;
	rghty.setYear(year);
	rghty.setMonth(mon);
	rghty.setDay(day);
